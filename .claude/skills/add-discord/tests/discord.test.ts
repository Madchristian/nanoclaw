...
describe('discord skill package', () => {
  ...
  it('modified index.ts includes Discord channel creation alongside Telegram', () => {
    const content = fs.readFileSync(
      path.join(skillDir, 'modify', 'src', 'index.ts'),
      'utf-8',
    );

    // Multi-channel architecture (all three channels)
    ...
    expect(content).toContain('if (!TELEGRAM_ONLY && !DISCORD_ONLY)');
  ...
  it('modified config.ts preserves all existing exports', () => {
    ...
    // Telegram exports preserved
    expect(content).toContain('export const TELEGRAM_BOT_TOKEN');
    expect(content).toContain('export const TELEGRAM_ONLY');
  });
